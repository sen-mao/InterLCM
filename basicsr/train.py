...
def create_train_val_dataloader(opt, logger):
    ...
    return train_loader, train_sampler, val_loader, total_epochs, total_iters, train_set
...
def train_pipeline(root_path):
    ...
    train_loader, train_sampler, val_loader, total_epochs, total_iters, train_set = result
    ...
    for epoch in range(start_epoch, total_epochs + 1):
        ...
        while train_data is not None:
            ...
            if opt['rank'] == 0 and current_iter % opt['logger']['print_freq'] == 0 or current_iter == 1:
                ...
                # LQ
                save_images(model.input, f'{save_path}/{iter}iter_lq.png')
                # HQ
                save_images(model.gt, f'{save_path}/{iter}iter_hq_iter.png')
...
def save_images(images, path):
    ...
    canvas = np.zeros((height, width * images.shape[0], 3))

    for i, image in enumerate(images):
        canvas[:, int(i*width):int((i+1)*width),:] = image

    canvas_image = Image.fromarray((canvas * 255).astype(np.uint8))
