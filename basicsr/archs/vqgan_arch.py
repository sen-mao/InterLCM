...
class VectorQuantizer(nn.Module):
    ...
    def get_codebook_feat(self, indices, shape):
        ...
        # shape: batch, N, channel (feature size)
        indices = indices.view(-1,1)
        min_encodings = torch.zeros(indices.shape[0], self.codebook_size).to(indices)
        min_encodings.scatter_(1, indices, 1)
        # get quantized latent vectors
        z_q = torch.matmul(min_encodings.float(), self.embedding.weight)  # (B*N, feature size): (B*256, 256)

        if shape is not None:  ## reshape back to match text embedding of LCM
            z_q = z_q.view(shape).contiguous()  # (B*N, feature size) -> (B, N, feature size)
...
#         blocks = []
#         # initial conv
#         blocks.append(nn.Conv1d(self.in_channels, block_in_ch, kernel_size=3, stride=1, padding=1))  # (B, N, feature size): (B, 256, 256) -> (B, 512, 256)
#
#         # non-local attention block
